...
const inactiveDriverMessage = 'यह ड्राइवर inactive है। कृपया Operations Office से संपर्क करें।';

const canDriverAccessApp = (driver?: DriverMaster | null) => {
  if (!driver) return false;
  const driverStatus = (driver.driverStatus || '').toString().toLowerCase();
  return (
    driver.otpActive === true &&
    (driver.recordStatus || 'Active') === 'Active' &&
...
const driverLightThemeStyles = `
  ...
  .dnk-driver-light .driver-biz-card {
    background: #E7F6EF !important;
    border-color: rgba(5, 150, 105, 0.28) !important;
    box-shadow: 0 8px 24px rgba(15, 23, 42, 0.08) !important;
  }

  .dnk-driver-light [class*="rounded-2xl"][class*="bg-slate-900"] {
    background: #E7F6EF !important;
    border-color: rgba(5, 150, 105, 0.24) !important;
    box-shadow: 0 8px 22px rgba(15, 23, 42, 0.07) !important;
  }

  .dnk-driver-light .driver-biz-surface {
    background: #F4FBF8 !important;
    border-color: rgba(5, 150, 105, 0.2) !important;
  }

  .dnk-driver-light .driver-biz-strong {
    background: #059669 !important;
    border-color: #047857 !important;
    color: #FFFFFF !important;
  }

  .dnk-driver-light .driver-biz-icon {
    background: #059669 !important;
    border-color: #047857 !important;
    color: #FFFFFF !important;
  }

  .dnk-driver-light .driver-biz-icon svg,
  .dnk-driver-light .driver-biz-strong svg {
    color: #FFFFFF !important;
  }

  .dnk-driver-light .driver-permission-chip {
    background: rgba(5, 150, 105, 0.1) !important;
    border-color: rgba(5, 150, 105, 0.22) !important;
    color: #065F46 !important;
  }

  .dnk-driver-light .driver-vehicle-number span,
  .dnk-driver-light .driver-vehicle-number strong {
    font-size: 0 !important;
  }

  .dnk-driver-light .driver-vehicle-number span::before {
    content: "गाड़ी नं / Vehicle Number";
    display: block;
    color: #334155;
    font-size: 12px;
    font-weight: 800;
  ...
  .dnk-driver-light .driver-vehicle-number strong::before {
...
export default function DriverApp({ currentUser, onLogout, onLoginSuccess, fullScreen = false }: DriverAppProps) {
  ...
  const [driverAccessMessage, setDriverAccessMessage] = useState('');
  const [driverHeaderExpanded, setDriverHeaderExpanded] = useState(false);
  ...
  useEffect(() => {
    ...
    const resolveDriverWork = () => {
      ...
      if (driverMaster && !canDriverAccessApp(driverMaster)) {
        setDriverAccessMessage(inactiveDriverMessage);
        setActiveTrip(null);
        setActiveLoadingConfirmation(null);
  ...
  const handleDriverOtpLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!canDriverAccessApp(driver)) {
        setDriverAccessMessage(inactiveDriverMessage);
        alert(inactiveDriverMessage);
        return;
      }
    ...
    <div className={driverFrameClass}>
      ...
      {!fullScreen && <div className="w-1/2 h-5 bg-slate-800 absolute top-0 left-1/4 rounded-b-xl z-20 flex items-center justify-center">
        ...
        <div className={driverShellClass}>
          <style>{driverLightThemeStyles}</style>
        {/* Navigation Head */}
        <div className="driver-biz-card px-3.5 pt-6 pb-3 border-b sticky top-0 z-10 flex flex-wrap items-center justify-between gap-2">
          <button
            type="button"
            onClick={() => setDriverHeaderExpanded(prev => !prev)}
            className="min-w-0 flex-1 flex items-center gap-2 rounded-xl px-1.5 py-1 text-left active:scale-[0.99] cursor-pointer"
            aria-expanded={driverHeaderExpanded}
          >
            <div className="w-9 h-9 rounded-full driver-biz-icon flex items-center justify-center font-black text-xs shrink-0">
              {currentUser.name.slice(0, 2).toUpperCase()}
            </div>
            <div className="min-w-0 flex-1">
              <h2 className="text-sm font-extrabold text-slate-100 truncate">DNK TRANS LOGISTICS</h2>
              <p className="text-[10px] text-slate-500 font-semibold truncate">Driver Portal</p>
              <p className="hidden">Driver ID: {currentDriverMaster?.driverCode || currentUser.uid.slice(-4)}</p>
              <p className="hidden">
                गाड़ी / Vehicle: {assignedTruckNumber || 'Not linked yet'}
              </p>
            </div>
            <ChevronDown className={`w-4 h-4 text-emerald-700 shrink-0 transition-transform ${driverHeaderExpanded ? 'rotate-180' : ''}`} />
          </button>
          ...
          {driverHeaderExpanded && (
            <div className="driver-biz-surface w-full rounded-xl border p-3 grid grid-cols-2 gap-2.5 text-left">
              <div>
                <span className="text-[9px] text-slate-500 font-bold uppercase tracking-wider block">Company</span>
                <strong className="text-[11px] text-slate-900 block truncate">DNK TRANS LOGISTICS</strong>
              </div>
              <div>
                <span className="text-[9px] text-slate-500 font-bold uppercase tracking-wider block">Driver Name</span>
                <strong className="text-[11px] text-slate-900 block truncate">{currentUser.name}</strong>
              </div>
              <div>
                <span className="text-[9px] text-slate-500 font-bold uppercase tracking-wider block">Mobile</span>
                <strong className="text-[11px] text-slate-900 block truncate">{currentUser.mobile || '-'}</strong>
              </div>
              <div>
                <span className="text-[9px] text-slate-500 font-bold uppercase tracking-wider block">Driver ID</span>
                <strong className="text-[11px] text-slate-900 block truncate">{currentDriverMaster?.driverCode || currentUser.uid.slice(-4)}</strong>
              </div>
              <div className="col-span-2">
                <span className="text-[9px] text-slate-500 font-bold uppercase tracking-wider block">गाड़ी नं / Vehicle Number</span>
                <strong className="text-[12px] text-slate-900 font-mono block truncate">{assignedTruckNumber || 'Not linked yet'}</strong>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="flex-grow overflow-y-auto px-3.5 py-3 space-y-4">
          ...
          <div className="driver-biz-card p-3 rounded-xl border space-y-2">
            <div className="flex items-center gap-1.5 text-emerald-700">
              <MapPin className="w-3.5 h-3.5 animate-pulse shrink-0" />
              <span className="font-semibold text-[10px] text-left">
                Current Location: {gpsSim.latitude}, {gpsSim.longitude}
              </span>
            </div>
            <button
              onClick={simulateGpsWalk}
              className="px-2 py-1 driver-biz-strong active:scale-95 rounded-md text-[10px] font-mono font-medium tracking-tight flex items-center justify-center gap-1 cursor-pointer"
            >
              <Map className="w-2.5 h-2.5" /> Open Map
            </button>
            <div className="grid grid-cols-1 gap-2">
              <span className="driver-permission-chip rounded-lg border px-2 py-1 text-[10px] font-bold flex items-center gap-1">
                <MapPin className="w-3 h-3" /> Location Permission: ON
              </span>
              <span className="driver-permission-chip rounded-lg border px-2 py-1 text-[10px] font-bold flex items-center gap-1">
                <Mic className="w-3 h-3" /> Microphone Permission: Allow
              </span>
            </div>
          </div>

          {!driverHeaderExpanded && (
          <div className="driver-biz-card p-3 rounded-xl border flex items-center justify-between">
            <div className="flex items-center gap-2 min-w-0">
              <div className="w-8 h-8 rounded-lg driver-biz-icon flex items-center justify-center">
                <Truck className="w-4 h-4" />
              </div>
              <div className="driver-vehicle-number min-w-0 text-left" data-vehicle-number={assignedTruckNumber || 'Not linked yet'}>
                <span className="text-[9px] text-slate-500 font-bold uppercase tracking-wider block">गाड़ी / Vehicle</span>
                <strong className="text-xs text-white font-mono block truncate" data-vehicle-number={assignedTruckNumber || 'Not linked yet'}>
                  गाड़ी / Vehicle: {assignedTruckNumber || 'Not linked yet'}
                </strong>
              </div>
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-4 text-slate-200">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <div className="bg-slate-900 p-2 rounded-xl border border-white/5 flex flex-col items-center justify-center gap-2">
                  <span className="driver-permission-chip rounded-lg border px-2 py-1 text-[10px] font-bold flex items-center gap-1">
                    <Mic className="w-3 h-3" /> Microphone Permission: Allow
                  </span>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-4 text-slate-200">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <div className="bg-slate-900 overflow-hidden flex flex-col items-center justify-center gap-2">
                  <span className="driver-permission-chip rounded-lg border px-2 py-1 text-[10px] font-bold flex items-center gap-1">
                    <Mic className="w-3 h-3" /> Microphone Permission: Allow
                  </span>
